#!/usr/bin/env python3
"""
LinkNet baseline for Carvana binary segmentation.

Drop-in comparison against U-Net, DeepLabV3+, and seGANmentation.
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Train LinkNet on Carvana (binary segmentation)')

    p.add_argument('--data-root', type=str, default='data/Carvana_resized',
                   help='Root of the Carvana dataset')
    p.add_argument('--outdir', type=str, default='outdir/linknet',
...
def main():
    ...
    # ---- Model (LinkNet with ResNet-34 backbone) ----
    model = smp.Linknet(
    ...
    print(f'[INFO] LinkNet (resnet34) loaded  –  '
