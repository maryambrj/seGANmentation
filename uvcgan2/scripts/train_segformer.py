#!/usr/bin/env python3
"""
SegFormer (mit-b0) baseline for Carvana binary segmentation.

Drop-in comparison against U-Net, DeepLabV3+, LinkNet, and seGANmentation.
Uses Hugging Face transformers SegformerForSemanticSegmentation.
...
# ---------------------------------------------------------------------------
def train_one_epoch(model, loader, criterion, optimizer, device, img_size):
    model.train()
    running_loss = 0.0
    running_dice = 0.0
    running_iou  = 0.0
    ...
    for images, masks, _ in loader:
        ...
        # SegFormer outputs logits at 1/4 resolution; upsample to match mask
...
@torch.no_grad()
def validate(model, loader, criterion, device, img_size, save_dir=None):
    model.eval()
    running_loss = 0.0
    running_dice = 0.0
    running_iou  = 0.0
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Train SegFormer on Carvana (binary segmentation)')

    p.add_argument('--data-root', type=str, default='data/Carvana_resized',
                   help='Root of the Carvana dataset')
    p.add_argument('--outdir', type=str, default='outdir/segformer',
...
def main():
    ...
    # ---- Model (SegFormer mit-b0, smallest variant) ----
    ...
    print(f'[INFO] SegFormer (mit-b0) loaded  –  '
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_metrics = train_one_epoch(
            model, train_loader, criterion, optimizer, device, args.img_size)
        ...
        val_metrics = validate(
            model, val_loader, criterion, device, args.img_size,
