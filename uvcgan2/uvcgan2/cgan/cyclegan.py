...
class CycleGANModel(ModelBase):
    ...
    def _setup_models(self, config):
        ...
        models['gen_ba'] = construct_generator( #Maryam
            config.generator,
            config.data.datasets[1].shape,
            config.data.datasets[0].shape,
            self.device
        ...
        if self.is_train:
            models['disc_a'] = construct_discriminator( #Maryam
    ...
    def _setup_optimizers(self, config):
        ...
        optimizers.gen = select_optimizer(
            itertools.chain(
                ...
                self.models.gen_ba.parameters()  #Maryam
            ),
        ...
        optimizers.disc = select_optimizer(
            itertools.chain(
                self.models.disc_a.parameters(), #Maryam
    ...
    def __init__(
        ...
        lambda_a = 10.0, lambda_b = 10.0,  #Maryam
        lambda_iou=10.0, lambda_dice=0, lambda_idt = 0.5
    ):
        # pylint: disable=too-many-arguments
        self.lambda_a   = lambda_a  #Maryam
        self.lambda_b   = lambda_b  #Maryam
        ...
        self.criterion_cycle  = torch.nn.L1Loss()  #Maryam
    ...
    def forward(self):
        # def simple_fwd(batch, gen_fwd):
        def simple_fwd(batch, gen_fwd, gen_bkw):
            if batch is None:
                return (None, None)

            fake = gen_fwd(batch)
            reco = gen_bkw(fake)  #Maryam

            # return fake
            return (fake, reco)

        # self.images.fake_b = simple_fwd(
        ...
        self.images.fake_a, self.images.reco_b = simple_fwd(  #Maryam
            self.images.real_b, self.models.gen_ba, self.models.gen_ab  #Maryam
        )
    ...
    def backward_discriminators(self):
        fake_a = self.pred_a_pool.query(self.images.fake_a)  #Maryam
        ...
        self.losses.disc_a = self.backward_discriminator_base(  #Maryam
            self.models.disc_a, self.images.real_a, fake_a   #Maryam
        )

    def backward_generators(self):
        lambda_idt = self.lambda_idt
        lambda_a   = self.lambda_a  #Maryam
        lambda_b   = self.lambda_b  #Maryam

        self.losses.gen_ab = self.criterion_gan(
            self.models.disc_b(self.images.fake_b), True
        )
        self.losses.gen_ba = self.criterion_gan(  #Maryam
            self.models.disc_a(self.images.fake_a), True
        )
        self.losses.cycle_a = lambda_a * self.criterion_cycle(  #Maryam
            self.images.reco_a, self.images.real_a
        )
        self.losses.cycle_b = lambda_b * self.criterion_cycle(  #Maryam
            self.images.reco_b, self.images.real_b
        )

        # loss = (
        #       self.losses.gen_ab + self.losses.iou
        # )
        loss = (
              self.losses.gen_ab  + self.losses.gen_ba
            + self.losses.cycle_a + self.losses.cycle_b 
            + self.losses.iou + self.losses.dice
        )

        if lambda_idt > 0:
            self.images.idt_b = self.models.gen_ab(self.images.real_b)
            self.losses.idt_b = lambda_idt * self.criterion_idt(
                self.images.idt_b, self.images.real_b
            )

            self.images.idt_a = self.models.gen_ba(self.images.real_a)  #Maryam
            self.losses.idt_a = lambda_a * lambda_idt * self.criterion_idt(  #Maryam
                self.images.idt_a, self.images.real_a  #Maryam
            )

            # loss += (self.losses.idt_b)
