...
class IoULoss(nn.Module):
    def forward(self, prediction, target):
        ...
        if prediction.nelement() == 0 or target.nelement() == 0:
        ...
        IoU = (intersection + 1e-6) / (union + 1e-6)  # Adding a small epsilon to avoid division by zero
...
class DiceCoefficient(nn.Module):
    def forward(self, prediction, target):
        ...
        intersection = (prediction * target).sum()
        dice_coeff = (2. * intersection + 1e-6) / (prediction.sum() + target.sum() + 1e-6)
...
class Specificity(nn.Module):
    def forward(self, prediction, target):
        ...
        specificity = true_negative / (actual_negative + 1e-6)
        return specificity
